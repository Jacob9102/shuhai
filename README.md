...
> **零第三方依赖**，Docker 一条命令部署，也可以打包成 Windows 桌面软件。
...
仓库里的 [`desktop/`](desktop) 附带了打包工具，能把整个服务做成**绿色免安装的 Windows 软件**：拷到任何一台 Windows 10/11 电脑，双击 `书海.exe` 就能用，目标机器不需要安装 Node。

```powershell
cd desktop
powershell -ExecutionPolicy Bypass -File build.ps1     # 产物在 ..\dist\书海
```

打包出来的程序以**独立窗口**运行（用系统自带的 Edge/Chrome 应用模式，没有标签页也没有地址栏），右下角带托盘图标，支持开机自启、桌面快捷方式、单实例；关掉窗口即整体退出。数据跟着程序目录走，书源和阅读进度都在 `data\` 里。

- 默认只监听 `127.0.0.1`，不触发 Windows 防火墙弹窗；要让手机访问，把 `config.ini` 里的 `host` 改成 `0.0.0.0`
- 没装 Edge 也没装 Chrome 的机器会自动退回默认浏览器开标签页，功能完全一样
- 打包细节和实现说明见 [`desktop/README.md`](desktop/README.md)
